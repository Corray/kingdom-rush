...
// kingdom-rush terminal mode entry (opt-in, `go build -tags term`)
//
// V2 默认 build 是 ebiten desktop (main.go),terminal 模式作为 V1.7 兼容保留。
//
...
func handleEvent(g *Game, ev tcell.Event, r *TermRenderer) (quit bool) {
	switch e := ev.(type) {
	case *tcell.EventKey:
		switch e.Key() {
