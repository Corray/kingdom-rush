...
| V17 | 第二章 21-30 | 关卡 20→30（Twilight 章接续爬升：Tesla/Sniper 秀场关、不对称双路、恢复大队、终局 Ragnarok）；技能树 4→5 节点（capstone ×3，总价 120 = 1.33× 新上限 90 恢复取舍）；菜单三列动态化；成就 +2（18 项）；Normal 首跑 30/30 零校准（V16 蓝图基线立功）、Hard 28/30；218 tests | `v17.0` | `efb24ce` |
...
**未竟项 / 留档：** endless wave 预览波间不可见（audit LOW，下波清场后才生成）；护盾 Boss 可被一击秒杀绕过（audit LOW，设计取舍）；卖塔退款按默认分支近似（分支 B cost 差异小，可接受）；itch.io 上传待用户 butler login（脚手架已入库：docs/itch-page.md + make itch-zip，butler 已装 ~/bin）。

---

## V17 — 第二章：关卡 21-30（已收尾 2026-07-06，tag `v17.0` @ `efb24ce`）

> V16 收尾补齐仿真蓝图矩阵后，内容扩张有了可靠平衡基线。现有 20 关是 V6 时代设计，未吃满 V13-V16 新系统（Tesla 反集群 / Sniper 反 Boss / 护甲治疗组合波 / 升级分支）——第二章 10 关按新系统组合设计。LMP L2（内容为主 + 技能树/菜单结构小改）。

### 启动决策（用户拍板 2026-07-06）

- **决策 A — 星星经济：树加第 5 节点。** 30 关 × 3★ = 90 星可赚 = V11 三树总价 90，「必须选 build」取舍失效。每职业树 4→5 节点（capstone 组合 perk，价 10 → 单树 40 / 三树总价 120 = 1.33× 新上限），取舍恢复。老存档前 4 节点数值不动（零回归）。
- **决策 B — 难度定位：接续爬升。** L21-30 接 L20 继续向上（unlock 链自然延续），终局内容定位；新系统在高压下才有存在感。
- **主题分区：** 第 5 地图主题 Twilight（L21+ 紫色系 tint）；Boss 行为 L21-30 维持 tier 3（召唤）。
- **成就：** +2 新成就（30 关全通 / 90 星），既有成就条件不动（防打破已解锁语义），仅 perfectionist 描述去掉过时的 "(all 3-star)"。

### Phase 计划

| Phase | 范围 | AC |
|-------|------|-----|
| **P1 技能树第 5 节点** | treeNodesPerClass 4→5；capstone 组合 perk ×3（Knight/Archer/Rogue）；价 10 | 预算算术更新；老存档 4 节点行为零回归；单测锁 |
| **P2 关卡 21-30 数据** | 10 新关双路 yaml（吃满新系统：Tesla 秀场集群关 / Sniper 秀场装甲关 / 恢复大队 / 不对称双路 / 终局大波）；Twilight 主题 tint | 30 关完整性测试（双路+汇流+unlock 链）；数据测试全绿 |
| **P3 菜单三列 + 成就** | menuRowAtPixel + drawLevelSelect 列数动态化（3 列）；成就 +2 | 30 关菜单可点可玩；hitbox 对齐；成就单测 |
| **P4 仿真 30 关 + 校准 + 收尾** | 仿真矩阵全量扩 30 关；校准循环；README/键位；tag `v17.0` | Normal 30/30（或合理弱项声明）；既有 20 关零回归；门面齐 |

### 已知翻车场景（盲点声明）

1. **L21-30 首跑不可通关概率高**——接续爬升数值全靠拍，V6 时代 10 关首跑也修过；对策：仿真循环 + 常量化
2. **三列菜单 stats 文案溢出**——colW 从 ~590 缩到 ~390，行内文案（星级+wave+gold+lives）可能挤爆；对策：压缩格式
3. **第 5 节点上界失控**——满树更强，V11「满树允许碾压 Hard」上界哲学不变，但不得破 Normal（满树矩阵守）
4. **term 菜单 30 关矮终端溢出**——term 冻结惯例，记录不阻塞

### V17 收尾记录（2026-07-06）

**完成度：** 4/4 phase 单 commit 交付（`efb24ce`），218 tests，三 build + vet 绿。

**盲点兑现：**
- 盲点 1（首跑不可通关）**反向**：Normal 首跑 30/30 全通零校准——V16 收尾建的蓝图矩阵基线立功，数值拍得准；难度分层由 Hard 承担（28/30，Lv9 老难点 + Lv23 新装甲难点，爬升合理）
- 盲点 2（stats 溢出）实锤：三列首拍右缘裁剪 → 压缩为 `w%d g%d` 收敛
- 盲点 3（第 5 节点上界）：满树 Hard 增益 K+1/A+1/R+0，Normal 满树全通不破。**Rogue 满树 +0 判定为结构性**（失守关 Lv9 wave1/Lv23 在英雄弱期或蓝图盲区，perk 救不动，V9「per-run 自限」同因）——Phantom 加强一档（+0.7 speed & ability 2s）后收手，与 V7.5「继续内推边际收益低」同判
- 盲点 4：未验证，term 冻结惯例留档
- **计划外发现**：树屏第 5 节点与帮助行重叠（panelH 硬编码 4 节点）→ 改按 treeNodesPerClass 动态

**平衡结论（30 关全景）：**

| 维度 | 数据 | 判定 |
|------|------|------|
| Normal | 30/30 全通 | 休闲基准保持 |
| Hard | 28/30（Lv9 / Lv23） | 两章各一难点关，爬升合理 |
| 蓝图矩阵 | 28-30/30，门槛 27 | 克制关系实证：sniper-mix 过装甲关 L23、tesla 蓝图 Boss 车轮 L29 失守 |
| 满树上下界 | 无树 = 基线零回归；满树 ≥ 无树 | 硬约束守住 |

**未竟项 / 留档：** Rogue capstone 长期观察（结构性 +0）；term 菜单 30 关未验证；itch.io 上传继续等 butler login。
...
| 2026-07-06 | itch.io 分发脚手架（`e16ec17`）：docs/itch-page.md 页面文案 + make itch-zip 可重复打包 + web/index.html 键位表 V7→V16；butler 已装 ~/bin，上传等用户 login（悬留项从"未启动"推进到"一步之遥"）|
| 2026-07-06 | V17 启动+交付+收尾（`efb24ce`）：用户拍板决策 A 树加第 5 节点（总价 120 恢复取舍）/ B 接续爬升。4 phase 单 commit：capstone ×3 + 关卡 21-30（Twilight）+ 菜单三列 + 成就 +2 + 仿真全矩阵扩 30。盲点 1 反向（首跑 30/30 零校准，V16 蓝图基线立功）；Rogue 满树 +0 判结构性收手。tag `v17.0` @ `efb24ce`。218 tests |
