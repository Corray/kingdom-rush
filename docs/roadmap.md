...
## V8 — 英雄单位（实现完成 2026-06-08，待用户实玩确认手感后 tag `v8.0`）
...
| 2026-06-08 | V8 实现完成：5/5 phase 交付（cdcff18/442ec2a/3a7ad60/6baa258 + P5），测试 129→148。平衡仿真证英雄是纯增量（Hard 17→19，无英雄基线零回归）。**待用户实玩确认手感后 tag `v8.0` + 部署**（手感是仿真测不出的、用户的判断）|
